...
class ChatApplication extends JFrame implements ActionListener {
    Container c;
    JPanel panel1;
    ...
    ChatApplication() {
        ...
        back.addMouseListener(new MouseAdapter() { //MouseAdapter is an abstract class.
            @Override
        ...
        ImageIcon image4 = new ImageIcon(ClassLoader.getSystemResource("krishna1.png"));  //Find a resource of the specified name from the search path used to load classes. This method locates the resource through the system class loader (see getSystemClassLoader()).
    ...
    public void actionPerformed(ActionEvent ae) {
        try {
            ...
            vertical.add(right);  // this will set the message in vertical row if multiple message is there.
...
public class ServerChat {
    public static void main(String[] args) {
        ChatApplication frame = new ChatApplication();
        ...
        try {
            ...
            while(true) {
                ...
                ChatApplication.pw = new PrintWriter(socket.getOutputStream());

                while (true) {
                    String message = br.readLine();
                    JPanel panel = ChatApplication.formatLabel(message);

                    JPanel left = new JPanel(new BorderLayout());
                    left.add(panel, BorderLayout.LINE_START);
                    ChatApplication.vertical.add(left);
                    frame.validate();
                }
            }
        }
